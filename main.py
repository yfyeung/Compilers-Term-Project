...
if __name__ == '__main__':
    ...
    for test_name in Configs.test_names: # 遍历测试文件
        ...
        token_table_path = os.path.join(Configs.dir_names["output"], "token_table", "token_table_" + test_name.replace(".sql", ".tsv"))
        input_stack_path = os.path.join(Configs.dir_names["output"], "input_stack", "input_stack_" + test_name.replace(".sql", ".tsv"))
        input_stack = token2inputStack(token_table_path, input_stack_path)
        save_path = os.path.join(Configs.dir_names["output"], "parse_result", "parse_result_" + test_name.replace(".sql", ".tsv"))
